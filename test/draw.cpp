...
void CDraw::Text(WCHAR *text, float x, float y, int orientation, int font, bool bordered, DWORD color, DWORD bcolor)
{
	RECT rect;
	...
	case lefted:
		if (bordered)
		{
			...
			pFont[font]->DrawText(NULL, text, -1, &rect, DT_LEFT | DT_NOCLIP, bcolor);
			SetRect(&rect, x + 1, y, x + 1, y);
			pFont[font]->DrawText(NULL, text, -1, &rect, DT_LEFT | DT_NOCLIP, bcolor);
		...
		pFont[font]->DrawText(NULL, text, -1, &rect, DT_LEFT | DT_NOCLIP, color);
		break;
	case centered:
		if (bordered)
		{
			SetRect(&rect, x - 1, y, x - 1, y);
			pFont[font]->DrawText(NULL, text, -1, &rect, DT_CENTER | DT_NOCLIP, bcolor);
			SetRect(&rect, x + 1, y, x + 1, y);
			pFont[font]->DrawText(NULL, text, -1, &rect, DT_CENTER | DT_NOCLIP, bcolor);
			SetRect(&rect, x, y - 1, x, y - 1);
			pFont[font]->DrawText(NULL, text, -1, &rect, DT_CENTER | DT_NOCLIP, bcolor);
		...
		SetRect(&rect, x, y, x, y);
		pFont[font]->DrawText(NULL, text, -1, &rect, DT_CENTER | DT_NOCLIP, color);
		break;
	case righted:
		if (bordered)
		{
			SetRect(&rect, x - 1, y, x - 1, y);
			pFont[font]->DrawText(NULL, text, -1, &rect, DT_RIGHT | DT_NOCLIP, bcolor);
			SetRect(&rect, x + 1, y, x + 1, y);
			pFont[font]->DrawText(NULL, text, -1, &rect, DT_RIGHT | DT_NOCLIP, bcolor);
			SetRect(&rect, x, y - 1, x, y - 1);
			pFont[font]->DrawText(NULL, text, -1, &rect, DT_RIGHT | DT_NOCLIP, bcolor);
		...
		SetRect(&rect, x, y, x, y);
		pFont[font]->DrawText(NULL, text, -1, &rect, DT_RIGHT | DT_NOCLIP, color);
		break;
	}
}
...
void CDraw::AddFont(LPCWSTR Caption, float size, bool bold, bool italic)
{
